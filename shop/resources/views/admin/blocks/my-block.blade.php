<div  style="padding: 20px; border: 1px solid black;margin: 10px 0">
    <div class="row">
        <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control" id="title-{{ $id }}" name="title" value="{{ $attributes['fields']['title'] ?? '' }}">
        </div>
        <div class="col-md-6 mb-3">
            <label for="subtitle-{{ $id }}" class="form-label">Subtitle</label>
            <input type="text" class="form-control" id="subtitle-{{ $id }}" name="subtitle" value="{{ $attributes['fields']['subtitle'] ?? '' }}">
        </div>
        <div class="col-md-12 mb-3">
            <label for="description-{{ $id }}" class="form-label">Description</label>
            <input type="text" id="description-{{ $id }}" class="form-control mce-editor" name="description" value="{{ $attributes['fields']['description'] ?? '' }}" >
        </div>
        ...
        <div class="col-md-12 mb-3">
            ...
            <div class="repeatable-block" style="border: 1px solid #d7d7d7; padding: 10px;" data-row='@json($repeater_html)'> <!-- class 'repeatable-block' for repeater initialization, data-row="html" html for one row -->
                <input class="repeater-values" type="hidden" name="list" value="{{ $attributes['fields']['list'] ?? '' }}" > <!-- class 'repeater-values' for a input for saving data from the repeater into it -->
